...
| `traffic_up` | number | 本条记录相对上一条记录的上行流量增量（字节）。旧记录可能为 0 |
| `traffic_down` | number | 本条记录相对上一条记录的下行流量增量（字节）。旧记录可能为 0 |
| `process` | number | 进程数量 |
...
::: tip 流量增量说明
`traffic_up` 和 `traffic_down` 是写入记录时计算的精确增量，定时流量报告会基于这两个字段求和。它们与 `net_total_up` / `net_total_down` 不同，后者是节点上报的累计计数器，可能因节点重启或计数器归零而变小。
:::

## 流量定时报告配置

以下接口需要后台认证。它们用于管理节点是否参与日报、周报、月报流量通知。

### GET `/api/admin/notification/traffic-report/`
...
| `enable` | boolean | 是否启用该节点的定时流量报告 |
| `daily` | boolean | 是否发送日报 |
| `weekly` | boolean | 是否发送周报 |
| `monthly` | boolean | 是否发送月报 |
| `client_info` | object | 关联节点信息，列表接口中可能返回 |

### POST `/api/admin/notification/traffic-report/edit`
...
如果 `enable=true`，必须至少设置 `daily`、`weekly`、`monthly` 中的一个为 `true`。
...
### POST `/api/admin/notification/traffic-report/enable`

批量启用已有的流量定时报告配置。请求体为节点 UUID 数组。
...
启用前需要该节点已经存在至少一种报告周期配置。若该节点没有日报、周报或月报配置，应使用 `/edit` 一次性提交 `enable` 和周期字段。

### POST `/api/admin/notification/traffic-report/disable`

批量禁用流量定时报告配置。请求体为节点 UUID 数组。
...
该接口只修改 `enable` 字段，不会清除日报、周报、月报选择。
