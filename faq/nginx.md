...
- **实在无法处理 ws 反向代理**：1.0.8 版本开始，只有终端强制使用 WebSocket，如果无法建立 ws 连接，会回落到 POST 请求（仅默认主题）。部分第三方主题可能需要 WebSocket 支持，此时可以在 `后台-设置-站点` 中关闭 `WebSocket Origin 校验`，或将实际访问地址加入 `WebSocket Origin 允许列表`。
